...
================Loon==============
[Script]
cron "50 8,20 * * *" script-path=https://raw.githubusercontent.com/xiaerol/personal/main/tttl.js, tag=太太乐

[task_local]
20 2,4,6 * * * http://47.101.146.160/scripts/.js, tag=太太乐, img-url=circles.hexagongrid.fill.system, enabled=true
*/
...
async function login(userName,password) {
    return new Promise((resolve) => {
        request.post(ttl('/login/restaurant', `{"mthd":"login","platform":"wx_mini","userName":"${userName}","password":"${password}"}`), function (error, response, body) {
            try {
                if (error) {
                    console.log(`${JSON.stringify(err)}`)
                    console.log(`${$.name} API请求失败，请检查网路重试`)
